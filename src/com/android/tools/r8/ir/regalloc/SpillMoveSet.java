...
class SpillMoveSet {
  ...
  public void addPhiMove(int i, LiveIntervals to, LiveIntervals from) {
    ...
    instructionToPhiMoves.computeIfAbsent(i, new Function<Integer, Set<SpillMove>>() {
      @Override
      public Set<SpillMove> apply(Integer k) {
  ...
  private void addInMove(int i, LiveIntervals to, LiveIntervals from) {
    assert i % 2 == 1;
    instructionToInMoves.computeIfAbsent(i, new Function<Integer, Set<SpillMove>>() {
      @Override
      public Set<SpillMove> apply(Integer k) {
        return new LinkedHashSet<>();
      }
  ...
  private void addOutMove(int i, LiveIntervals to, LiveIntervals from) {
    assert i % 2 == 1;
    instructionToOutMoves.computeIfAbsent(i, new Function<Integer, Set<SpillMove>>() {
      @Override
      public Set<SpillMove> apply(Integer k) {
        return new LinkedHashSet<>();
      }
  ...
  private void scheduleMovesBeforeInstruction(
    ...
    Set<SpillMove> inMoves =
        instructionToInMoves.computeIfAbsent(instruction - 1, new Function<Integer, Set<SpillMove>>() {
          @Override
          public Set<SpillMove> apply(Integer k) {
            return new LinkedHashSet<>();
          }
    ...
    Set<SpillMove> outMoves =
        instructionToOutMoves.computeIfAbsent(instruction - 1, new Function<Integer, Set<SpillMove>>() {
          @Override
          public Set<SpillMove> apply(Integer k) {
            return new LinkedHashSet<>();
          }
    ...
    Set<SpillMove> phiMoves =
        instructionToPhiMoves.computeIfAbsent(instruction - 1, new Function<Integer, Set<SpillMove>>() {
          @Override
          public Set<SpillMove> apply(Integer k) {
            return new LinkedHashSet<>();
          }
