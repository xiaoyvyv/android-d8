...
public class LinearScanRegisterAllocator implements RegisterAllocator {
  ...
  protected PriorityQueue<LiveIntervals> unhandled =
      new PriorityQueue<>(Comparator.comparingInt(new ToIntFunction<LiveIntervals>() {
        @Override
        public int applyAsInt(LiveIntervals t) {
          return t.getStart();
        }
      }));
  ...
  private void computeDebugInfo(BasicBlock[] blocks) {
    ...
    for (LiveIntervals interval : liveIntervals) {
      ...
      List<Integer> starts = ListUtils.map(value.getDebugLocalStarts(), new Function<Instruction, Integer>() {
        @Override
        public Integer apply(Instruction instruction) {
          return instruction.getNumber();
        }
      });
      List<Integer> ends = ListUtils.map(value.getDebugLocalEnds(), new Function<Instruction, Integer>() {
        @Override
        public Integer apply(Instruction instruction) {
          return instruction.getNumber();
        }
      ...
      liveRanges.sort(new Comparator<LiveRange>() {
        @Override
      ...
      starts.sort(new Comparator<Integer>() {
        @Override
        public int compare(Integer x1, Integer y1) {
    ...
    ranges.sort(new Comparator<LocalRange>() {
      @Override
      public int compare(LocalRange localRange, LocalRange o) {
        return localRange.compareTo(o);
      }
    });
    ...
    for (LiveIntervals intervals : liveIntervals) {
      if (intervals.hasSplits()) {
        ...
        PriorityQueue<LiveIntervals> sortedChildren =
            new PriorityQueue<>(new Comparator<LiveIntervals>() {
              @Override
              public int compare(LiveIntervals o1, LiveIntervals o2) {
  ...
  private void clearUserInfo() {
    code.blocks.forEach(new Consumer<BasicBlock>() {
      @Override
      public void accept(BasicBlock basicBlock) {
        basicBlock.clearUserInfo();
      }
    });
  }
