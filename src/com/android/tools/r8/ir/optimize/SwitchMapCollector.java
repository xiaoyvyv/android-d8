...
public class SwitchMapCollector {
  ...
  public void run() {
    appInfo.classes().forEach(new Consumer<DexProgramClass>() {
      @Override
      public void accept(DexProgramClass clazz) {
        SwitchMapCollector.this.processClasses(clazz);
      }
    });
  ...
  private void processClasses(DexProgramClass clazz) {
    ...
    List<DexEncodedField> switchMapFields = Arrays.stream(clazz.staticFields())
        .filter(new Predicate<DexEncodedField>() {
          @Override
          public boolean test(DexEncodedField dexEncodedField) {
            return SwitchMapCollector.this.maybeIsSwitchMap(dexEncodedField);
          }
        }).collect(Collectors.toList());
    if (!switchMapFields.isEmpty()) {
      final IRCode initializer = clazz.getClassInitializer().buildIR(options);
      switchMapFields.forEach(new Consumer<DexEncodedField>() {
        @Override
        public void accept(DexEncodedField field) {
          SwitchMapCollector.this.extractSwitchMap(field, initializer);
        }
      });
    }
  }
  ...
  private void extractSwitchMap(DexEncodedField encodedField, IRCode initializer) {
    ...
    Instruction insn;
    Predicate<Instruction> predicate = new Predicate<Instruction>() {
      @Override
      public boolean test(Instruction i) {
        return i.isStaticGet() && i.asStaticGet().getField() == field;
      }
    };
