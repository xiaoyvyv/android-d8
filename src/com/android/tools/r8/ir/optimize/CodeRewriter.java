...
public class CodeRewriter {
  ...
  private static boolean removedTrivialGotos(IRCode code) {
    ...
    do {
      ...
      assert !block.isTrivialGoto()
          ...
          || block.getPredecessors().stream().anyMatch(new Predicate<BasicBlock>() {
        @Override
        public boolean test(BasicBlock b) {
          return b.exit().fallthroughBlock() == blk;
        }
      });
        ...
        if (insn.isSwitch()) {
          ...
          if (info != null) {
            ...
            if (arrayGet.outValue().numberOfUsers() == 0) {
              arrayGet.inValues().forEach(new Consumer<Value>() {
                @Override
                public void accept(Value v) {
                  v.removeUser(arrayGet);
                }
              });
      ...
      while (iterator.hasPrevious()) {
        ...
        if (current.isStaticGet()) {
          ...
          if (dominatingPuts.containsKey(field)) {
            ...
            Iterables.removeIf(puts, new com.google.common.base.Predicate<StaticPut>() {
              @Override
              public boolean apply(StaticPut put) {
                return put.getField() == field;
              }
            });
          }
        }
  ...
  public void removeCastChains(IRCode code) {
    ...
    while (it.hasNext()) {
      ...
      if (current.isCheckCast()
        ...
        final CheckCast checkCast = current.asCheckCast();
        if (checkCast.outValue().uniqueUsers().stream().allMatch(
                new Predicate<Instruction>() {
                  @Override
                  public boolean test(Instruction user) {
                    return user.isCheckCast()
      ...
      while (iterator.hasNext()) {
        ...
        if (canBeFolded(current)) {
          ...
          folded.outValue().uniqueUsers()
              .forEach(new Consumer<Instruction>() {
                @Override
                public void accept(Instruction instruction) {
                  worklist.add(instruction.getBlock());
                }
              });
        }
      }
  ...
  private void insertConstantInBlock(Instruction instruction, BasicBlock block) {
    ...
    insertAt.nextUntil(new Predicate<Instruction>() {
      @Override
  ...
  private ConstInstruction[] computeConstantArrayValues(
    ...
    InstructionListIterator it = block.listIterator();
    it.nextUntil(new Predicate<Instruction>() {
      @Override
      public boolean test(Instruction i) {
        return i == newArray;
      }
  ...
  public void logArgumentTypes(DexEncodedMethod method, IRCode code) {
    ...
    iterator.nextUntil(new Predicate<Instruction>() {
      @Override
