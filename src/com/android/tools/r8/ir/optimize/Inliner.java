  ...
  public synchronized void processDoubleInlineCallers(IRConverter converter,
    ...
    if (doubleInlineCallers.size() > 0) {
      ...
      List<DexEncodedMethod> methods = doubleInlineCallers
          .stream()
          .sorted(new Comparator<DexEncodedMethod>() {
            @Override
            public int compare(DexEncodedMethod m1, DexEncodedMethod m2) {
              return DexEncodedMethod.slowCompare(m1, m2);
            }
          })
          .collect(Collectors.toList());
      for (DexEncodedMethod method : methods) {
        converter.processMethod(method, feedback, new BiConsumer<IRCode, DexEncodedMethod>() {
          @Override
