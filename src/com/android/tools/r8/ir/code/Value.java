...
public class Value {
  ...
  public void replaceUsers(Value newValue) {
    ...
    for (Instruction user : uniqueUsers()) {
      user.inValues.replaceAll(new UnaryOperator<Value>() {
        @Override
    ...
    for (Phi user : uniquePhiUsers()) {
      user.getOperands().replaceAll(new UnaryOperator<Value>() {
        @Override
        public Value apply(Value v) {
          if (v == Value.this) {
    ...
    if (debugData != null) {
      for (Instruction user : debugUsers()) {
        user.getDebugValues().replaceAll(new UnaryOperator<Value>() {
          @Override
          public Value apply(Value v) {
            if (v == Value.this) {
