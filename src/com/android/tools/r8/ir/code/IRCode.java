...
public class IRCode {
    ...
    public boolean consistentBlockNumbering() {
        Map<Object, Long> collect = blocks.stream()
                .collect(Collectors.groupingBy(new Function<BasicBlock, Object>() {
                    @Override
                    public Object apply(BasicBlock basicBlock) {
                        return basicBlock.getNumber();
                    }
                }, Collectors.counting()));

        return collect.entrySet().stream().noneMatch(new Predicate<Map.Entry<Object, Long>>() {
            @Override
            public boolean test(Map.Entry<Object, Long> bb2count) {
                return bb2count.getValue() > 1;
            }
        });
    }
    ...
    public final int getHighestBlockNumber() {
        return blocks.stream().max(Comparator.comparingInt(new ToIntFunction<BasicBlock>() {
            @Override
            public int applyAsInt(BasicBlock t) {
                return t.getNumber();
            }
        })).get().getNumber();
    }
