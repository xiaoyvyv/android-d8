...
public class JarSourceCode implements SourceCode {
  ...
  private static Map<LocalVariableNode, DebugLocalInfo> computeLocals(
    ...
    for (Object o : localNodes) {
      ...
      localVariables.computeIfAbsent(node, new Function<LocalVariableNode, DebugLocalInfo>() {
        @Override
        public DebugLocalInfo apply(LocalVariableNode n) {
          return canonicalizeLocal(n, canonical, application);
        }
      });
    }
    ...
    for (JarStateWorklistItem item = worklist.poll(); item != null; item = worklist.poll()) {
      ...
      for (int i = item.instructionIndex; i <= instructionCount(); ++i) {
        ...
        if (i == instructionCount() || (i != item.instructionIndex && CFG.containsKey(i))) {
          ...
          item.blockInfo.normalSuccessors.iterator().forEachRemaining(new IntConsumer() {
            @Override
            public void accept(int offset) {
              if (state.recordStateForTarget(offset, JarSourceCode.this)) {
                if (offset >= 0) {
                  worklist.add(new JarStateWorklistItem(CFG.get(offset), offset));
                }
              }
            }
          });
          ...
          item.blockInfo.exceptionalSuccessors.iterator().forEachRemaining(new IntConsumer() {
            @Override
            public void accept(int offset) {
              if (state.recordStateForExceptionalTarget(offset, JarSourceCode.this)) {
                if (offset >= 0) {
                  worklist.add(new JarStateWorklistItem(CFG.get(offset), offset));
                }
              }
  ...
  private void build(MethodInsnNode insn, IRBuilder builder) {
    ...
    buildInvoke(
        ...
        builder,
            new BiConsumer<List<MoveType>, List<Integer>>() {
              @Override
              public void accept(List<MoveType> types, List<Integer> registers) {
                Invoke.Type invokeType = JarSourceCode.this.invokeType(insn);
