...
public class IRConverter {
  ...
  private Set<DexMethod> libraryMethodsReturningReceiver() {
    ...
    dexItemFactory.stringBufferMethods.forEachAppendMethod(new Consumer<DexMethod>() {
      @Override
      public void accept(DexMethod e) {
        methods.add(e);
      }
    });
    dexItemFactory.stringBuilderMethods.forEachAppendMethod(new Consumer<DexMethod>() {
      @Override
      public void accept(DexMethod e) {
        methods.add(e);
      }
  ...
  private void convertClassesToDex(Iterable<DexProgramClass> classes,
    ...
    for (final DexProgramClass clazz : classes) {
        ...
        public void run() {
          clazz.forEachMethod(new Consumer<DexEncodedMethod>() {
                                ...
                                public void accept(DexEncodedMethod dexEncodedMethod) {
                                  IRConverter.this.convertMethodToDex(dexEncodedMethod);
                                }
                              });
        }
      }));
    ...
    if (method.getCode() != null) {
      ...
      if (matchesMethodFilter) {
        if (method.getCode().isJarCode()) {
          rewriteCode(method, ignoreOptimizationFeedback, new BiConsumer<IRCode, DexEncodedMethod>() {
            @Override
            public void accept(IRCode code, DexEncodedMethod method1) {
              Outliner.noProcessing(code, method1);
            }
          });
        }
  ...
  public DexApplication optimize(ExecutorService executorService) throws ExecutionException {
    ...
    callGraph.forEachMethod(new Consumer<DexEncodedMethod>() {
      @Override
      public void accept(DexEncodedMethod method) {
        if (outliner == null) IRConverter.this.processMethod(method, directFeedback,
                new BiConsumer<IRCode, DexEncodedMethod>() {
                  @Override
                  public void accept(IRCode code, DexEncodedMethod method1) {
                    Outliner.noProcessing(code, method1);
                  }
                });
        else IRConverter.this.processMethod(method, directFeedback,
                new BiConsumer<IRCode, DexEncodedMethod>() {
                  @Override
                  public void accept(IRCode code, DexEncodedMethod method1) {
                    outliner.identifyCandidates(code, method1);
                  }
                });
      }
    }, executorService);
      ...
      if (outlineClass != null) {
        ...
        callGraph.forEachMethod(new Consumer<DexEncodedMethod>() {
          ...
          public void accept(DexEncodedMethod method) {
            ...
            IRConverter.this.processMethod(method, ignoreOptimizationFeedback, new BiConsumer<IRCode, DexEncodedMethod>() {
              @Override
              public void accept(IRCode code, DexEncodedMethod method1) {
                outliner.applyOutliningCandidate(code, method1);
              }
            });
  ...
  private void clearDexMethodCompilationState() {
    application.classes().forEach(new Consumer<DexProgramClass>() {
      ...
      public void accept(DexProgramClass clazz) {
        IRConverter.this.clearDexMethodCompilationState(clazz);
      }
    });
  }
  ...
  public void optimizeSynthesizedClass(DexProgramClass clazz) {
    ...
    clazz.forEachMethod(new Consumer<DexEncodedMethod>() {
      @Override
      public void accept(DexEncodedMethod method) {
        IRConverter.this.optimizeSynthesizedMethod(method);
      }
    });
  }
  ...
  public void optimizeSynthesizedMethod(DexEncodedMethod method) {
    ...
    processMethod(method, ignoreOptimizationFeedback, new BiConsumer<IRCode, DexEncodedMethod>() {
      @Override
      public void accept(IRCode code, DexEncodedMethod method1) {
        Outliner.noProcessing(code, method1);
      }
