...
final class AccessorMethodSourceCode extends SynthesizedLambdaSourceCode {
  ...
  protected void prepareInstructions() {
    ...
    if (delegatingToConstructor()) {
      ...
      add(new Consumer<IRBuilder>() {
        ...
        public void accept(IRBuilder builder) {
          builder.addNewInstance(instance, implMethod.holder);
        }
      });
    ...
    add(new Consumer<IRBuilder>() {
      @Override
      public void accept(IRBuilder builder) {
        builder.addInvoke(AccessorMethodSourceCode.this.inferInvokeType(),
    ...
    if (proto.returnType == factory().voidType) {
      ...
      add(new Consumer<IRBuilder>() {
        @Override
        public void accept(IRBuilder builder) {
          builder.addReturn(MoveType.OBJECT, argRegisters.get(0));
        }
      });
    } else {
