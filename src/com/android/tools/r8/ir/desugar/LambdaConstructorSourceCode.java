  ...
  protected void prepareInstructions() {
    ...
    add(new Consumer<IRBuilder>() {
      ...
      public void accept(IRBuilder builder) {
        builder.addInvoke(Invoke.Type.DIRECT, objectInitMethod,
                objectInitMethod.proto, Collections.singletonList(LambdaConstructorSourceCode.this.getReceiverValue()));
      }
    });
    ...
    if (capturedValues > 0) {
      for (int i = 0; i < capturedValues; i++) {
        ...
        add(new Consumer<IRBuilder>() {
          @Override
          public void accept(IRBuilder builder) {
            builder.addInstancePut(memberType,
                    LambdaConstructorSourceCode.this.getParamRegister(idx), LambdaConstructorSourceCode.this.getReceiverRegister(), field);
          }
        });
      }
    }
