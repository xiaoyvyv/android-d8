...
public class MemberRebindingAnalysis {
    ...
    public GraphLense run() {
        computeMethodRebinding(appInfo.virtualInvokes, new Function<DexMethod, DexEncodedMethod>() {
                    @Override
                    public DexEncodedMethod apply(DexMethod method) {
                        return MemberRebindingAnalysis.this.virtualLookup(method);
                    }
                },
                new BiFunction<DexClass, DexMethod, DexEncodedMethod>() {
                    @Override
                    public DexEncodedMethod apply(DexClass dexClass, DexMethod method) {
                        return dexClass.findVirtualTarget(method);
                    }
                }, new BiConsumer<DexProgramClass, DexEncodedMethod>() {
                    @Override
                    public void accept(DexProgramClass dexProgramClass, DexEncodedMethod virtualMethod) {
                        dexProgramClass.addVirtualMethod(virtualMethod);
                    }
                });
        computeMethodRebinding(appInfo.superInvokes, new Function<DexMethod, DexEncodedMethod>() {
            @Override
            public DexEncodedMethod apply(DexMethod method1) {
                return MemberRebindingAnalysis.this.superLookup(method1);
            }
        }, new BiFunction<DexClass, DexMethod, DexEncodedMethod>() {
            @Override
            public DexEncodedMethod apply(DexClass dexClass, DexMethod method1) {
                return dexClass.findVirtualTarget(method1);
            }
        }, new BiConsumer<DexProgramClass, DexEncodedMethod>() {
            @Override
            public void accept(DexProgramClass dexProgramClass, DexEncodedMethod dexEncodedMethod) {
                dexProgramClass.addVirtualMethod(dexEncodedMethod);
            }
        });


        computeMethodRebinding(appInfo.directInvokes, new Function<DexMethod, DexEncodedMethod>() {
                    @Override
                    public DexEncodedMethod apply(DexMethod method) {
                        return appInfo.lookupDirectTarget(method);
                    }
                }
                ,
                new BiFunction<DexClass, DexMethod, DexEncodedMethod>() {
                    @Override
                    public DexEncodedMethod apply(DexClass dexClass, DexMethod method) {
                        return dexClass.findDirectTarget(method);
                    }
                }, new BiConsumer<DexProgramClass, DexEncodedMethod>() {
                    @Override
                    public void accept(DexProgramClass clazz, DexEncodedMethod method) {
                        privateMethodsCheck(clazz, method);
                    }
                });
        computeMethodRebinding(appInfo.staticInvokes, new Function<DexMethod, DexEncodedMethod>() {
                    @Override
                    public DexEncodedMethod apply(DexMethod method) {
                ...
                new BiFunction<DexClass, DexMethod, DexEncodedMethod>() {
                    @Override
                    public DexEncodedMethod apply(DexClass dexClass, DexMethod dexMethod) {
                        return dexClass.findDirectTarget(dexMethod);
                    }
                }, new BiConsumer<DexProgramClass, DexEncodedMethod>() {
                    @Override
                    public void accept(DexProgramClass dexProgramClass, DexEncodedMethod staticMethod) {
                        dexProgramClass.addStaticMethod(staticMethod);
                    }
                });

        computeFieldRebinding(Sets.union(appInfo.staticFieldReads, appInfo.staticFieldWrites),
                new BiFunction<DexType, DexField, DexEncodedField>() {
                    @Override
                    public DexEncodedField apply(DexType type, DexField field) {
                        return appInfo.lookupStaticTarget(type, field);
                    }
                }, new BiFunction<DexClass, DexField, DexEncodedField>() {
                    @Override
                    public DexEncodedField apply(DexClass dexClass, DexField field) {
                        return dexClass.findStaticTarget(field);
                    }
                });
        computeFieldRebinding(Sets.union(appInfo.instanceFieldReads, appInfo.instanceFieldWrites),
                new BiFunction<DexType, DexField, DexEncodedField>() {
                    ...
                    @Override
                    public DexEncodedField apply(DexClass dexClass, DexField field) {
                        return dexClass.findInstanceTarget(field);
                    }
                });
