...
public class VisibilityBridgeRemover {
  ...
  private void removeUnneededVisibilityBridges() {
    Set<DexType> classes = unneededVisibilityBridges.stream()
        .map(new Function<DexEncodedMethod, DexType>() {
          @Override
          public DexType apply(DexEncodedMethod method) {
            return method.method.getHolder();
          }
        })
  ...
  private DexEncodedMethod[] removeMethods(DexEncodedMethod[] methods,
    ...
    List<DexEncodedMethod> newMethods = Arrays.stream(methods)
        .filter(new Predicate<DexEncodedMethod>() {
          @Override
          public boolean test(DexEncodedMethod method) {
            return !removals.contains(method);
          }
        })
    ...
    for (DexClass clazz : appInfo.classes()) {
      clazz.forEachMethod(new Consumer<DexEncodedMethod>() {
        ...
        public void accept(DexEncodedMethod method) {
          VisibilityBridgeRemover.this.identifyBridgeMethod(method);
        }
      });
    }
