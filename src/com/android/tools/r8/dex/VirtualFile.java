...
public class VirtualFile {
  ...
  private static Map<DexProgramClass, String> computeOriginalNameMapping(
    ...
    classes.forEach(new Consumer<DexProgramClass>() {
      @Override
      public void accept(DexProgramClass c) {
        originalNames.put(c,
  ...
  public abstract static class DistributorBase extends Distributor {
    ...
    TreeSet<DexProgramClass> sortClassesByPackage(Set<DexProgramClass> classes,
      ...
      TreeSet<DexProgramClass> sortedClasses = new TreeSet<>(
              new Comparator<DexProgramClass>() {
                @Override
                public int compare(DexProgramClass a, DexProgramClass b) {
  ...
  public static class FillFilesDistributor extends DistributorBase {
    ...
    public Map<Integer, VirtualFile> run() throws ExecutionException, IOException {
      ...
      if (minimalMainDex && !mainDexFile.isEmpty()) {
        ...
        filesForDistribution = Maps.filterKeys(filesForDistribution, new Predicate<Integer>() {
          @Override
  ...
  public static class PackageMapDistributor extends DistributorBase {
    ...
    private Set<String> fillForDistribution(Set<DexProgramClass> classes,
      ...
      if (packageDistribution != null) {
        ...
        ThreadUtils.awaitFutures(futures).forEach(new Consumer<List<DexProgramClass>>() {
          @Override
          public void accept(List<DexProgramClass> collection) {
            classes.removeAll(collection);
          }
        });
      }
      return usedPrefixes;
  ...
  private static class IndexedItemTransaction implements IndexedItemCollection {
    ...
    void commit() {
      commitItemsIn(classes, new Function<DexProgramClass, Boolean>() {
        @Override
        public Boolean apply(DexProgramClass clazz) {
          return base.addClass(clazz);
        }
      });
      commitItemsIn(fields, new Function<DexField, Boolean>() {
        @Override
        public Boolean apply(DexField field) {
          return base.addField(field);
        }
      });
      commitItemsIn(methods, new Function<DexMethod, Boolean>() {
        @Override
        public Boolean apply(DexMethod method) {
          return base.addMethod(method);
        }
      });
      commitItemsIn(protos, new Function<DexProto, Boolean>() {
        @Override
        public Boolean apply(DexProto proto) {
          return base.addProto(proto);
        }
      });
      commitItemsIn(types, new Function<DexType, Boolean>() {
        @Override
        public Boolean apply(DexType type) {
          return base.addType(type);
        }
      });
      commitItemsIn(strings, new Function<DexString, Boolean>() {
        @Override
        public Boolean apply(DexString string) {
          return base.addString(string);
        }
      });
      commitItemsIn(callSites, new Function<DexCallSite, Boolean>() {
        @Override
        public Boolean apply(DexCallSite callSite) {
          return base.addCallSite(callSite);
        }
      });
      commitItemsIn(methodHandles, new Function<DexMethodHandle, Boolean>() {
        @Override
        public Boolean apply(DexMethodHandle methodHandle) {
          return base.addMethodHandle(methodHandle);
        }
      });
    }
