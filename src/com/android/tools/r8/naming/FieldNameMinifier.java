...
class FieldNameMinifier {
  ...
  Map<DexField, DexString> computeRenaming(Timing timing) {
    ...
    DexType.forAllInterfaces(appInfo.dexItemFactory,
            new Consumer<DexType>() {
              ...
              public void accept(DexType iface) {
                FieldNameMinifier.this.reserveNamesInSubtypes(iface, rootState);
              }
            });
    timing.end();
    // Now rename the rest.
    ...
    DexType.forAllInterfaces(appInfo.dexItemFactory, new Consumer<DexType>() {
      ...
      public void accept(DexType type) {
        FieldNameMinifier.this.renameFieldsInSubtypes(type);
      }
    });
    timing.end();
    return renaming;
  ...
  private void reserveNamesInSubtypes(DexType type, NamingState<DexType> state) {
    ...
    NamingState<DexType> newState = states.computeIfAbsent(type, new Function<DexType, NamingState<DexType>>() {
      @Override
      public NamingState<DexType> apply(DexType t) {
        return state.createChild();
      }
    });
    holder.forEachField(new Consumer<DexEncodedField>() {
      @Override
      public void accept(DexEncodedField field) {
        FieldNameMinifier.this.reserveFieldName(field, newState, holder.isLibraryClass());
      }
    });
    type.forAllExtendsSubtypes(new Consumer<DexType>() {
      @Override
      public void accept(DexType subtype) {
        FieldNameMinifier.this.reserveNamesInSubtypes(subtype, newState);
      }
    });
  }
  ...
  private void renameFieldsInSubtypes(DexType type) {
    ...
    final NamingState<DexType> state = states.get(clazz.type);
    assert state != null;
    clazz.forEachField(new Consumer<DexEncodedField>() {
      @Override
      public void accept(DexEncodedField field) {
        FieldNameMinifier.this.renameField(field, state);
      }
    });
    type.forAllExtendsSubtypes(new Consumer<DexType>() {
      @Override
      public void accept(DexType type1) {
        FieldNameMinifier.this.renameFieldsInSubtypes(type1);
      }
    });
  }
