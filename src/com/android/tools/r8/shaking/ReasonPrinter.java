...
public class ReasonPrinter {
  ...
  public void run(DexApplication application) {
    ...
    for (DexClass clazz : application.classes()) {
      ...
      Arrays.stream(clazz.staticFields()).filter(new Predicate<DexEncodedField>() {
        @Override
        public boolean test(DexEncodedField o) {
          return itemsQueried.contains(o);
        }
      })
          .forEach(new Consumer<DexEncodedField>() {
            @Override
            public void accept(DexEncodedField item) {
              printReasonFor(item);
            }
          });
      Arrays.stream(clazz.instanceFields()).filter(new Predicate<DexEncodedField>() {
        @Override
        public boolean test(DexEncodedField o) {
          return itemsQueried.contains(o);
        }
      ...
      Arrays.stream(clazz.directMethods()).filter(new Predicate<DexEncodedMethod>() {
        @Override
        public boolean test(DexEncodedMethod o) {
          return itemsQueried.contains(o);
        }
      })
          .forEach(new Consumer<DexEncodedMethod>() {
            @Override
            public void accept(DexEncodedMethod item) {
              printReasonFor(item);
            }
          });
      Arrays.stream(clazz.virtualMethods()).filter(new Predicate<DexEncodedMethod>() {
        @Override
        public boolean test(DexEncodedMethod o) {
          return itemsQueried.contains(o);
        }
