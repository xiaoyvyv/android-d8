...
public class AnnotationRemover {
  ...
  public void run() {
    ...
    for (DexProgramClass clazz : appInfo.classes()) {
      clazz.annotations = stripAnnotations(clazz.annotations, new Predicate<DexAnnotation>() {
        @Override
        public boolean test(DexAnnotation annotation) {
          return AnnotationRemover.this.filterAnnotations(annotation);
        }
      });
      clazz.forEachMethod(new Consumer<DexEncodedMethod>() {
        @Override
        public void accept(DexEncodedMethod method) {
          AnnotationRemover.this.processMethod(method);
        }
      });
      clazz.forEachField(new Consumer<DexEncodedField>() {
        @Override
        public void accept(DexEncodedField field) {
          AnnotationRemover.this.processField(field);
        }
      });
    }
  }
  ...
  private void processMethod(DexEncodedMethod method) {
    method.annotations = stripAnnotations(method.annotations, new Predicate<DexAnnotation>() {
      @Override
      public boolean test(DexAnnotation annotation) {
        return AnnotationRemover.this.filterAnnotations(annotation);
      }
    ...
    method.parameterAnnotations = stripAnnotations(method.parameterAnnotations,
            new Predicate<DexAnnotation>() {
              @Override
              public boolean test(DexAnnotation annotation) {
                return AnnotationRemover.this.filterParameterAnnotations(annotation);
              }
            });
  }

  private void processField(DexEncodedField field) {
      field.annotations = stripAnnotations(field.annotations, new Predicate<DexAnnotation>() {
        @Override
        public boolean test(DexAnnotation annotation) {
          return AnnotationRemover.this.filterAnnotations(annotation);
        }
  ...
  private DexAnnotationSetRefList stripAnnotations(DexAnnotationSetRefList annotations,
    ...
    if (filtered == null) {
      ...
      if (Arrays.stream(filtered).allMatch(new Predicate<DexAnnotationSet>() {
        @Override
        public boolean test(DexAnnotationSet dexAnnotationSet) {
          return dexAnnotationSet.isEmpty();
        }
      })) {
