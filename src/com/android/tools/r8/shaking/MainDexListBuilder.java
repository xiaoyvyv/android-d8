...
public class MainDexListBuilder {
  ...
  public MainDexListBuilder(Set<DexType> baseClasses, DexApplication application) {
    ...
    this.baseClasses =
        baseClasses.stream().filter(new Predicate<DexType>() {
          @Override
          public boolean test(DexType dexType) {
            return MainDexListBuilder.this.isProgramClass(dexType);
          }
  ...
  public Set<DexType> run() {
    ...
    return mainDexTypes.stream().filter(new Predicate<DexType>() {
      @Override
      public boolean test(DexType dexType) {
        return MainDexListBuilder.this.isProgramClass(dexType);
      }
    ...
    for (DexType type : baseClasses) {
      ...
      clazz.forEachField(new Consumer<DexEncodedField>() {
        ...
        public void accept(DexEncodedField field) {
          MainDexListBuilder.this.addMainDexType(field.field.type);
        }
      });
      clazz.forEachMethod(new Consumer<DexEncodedMethod>() {
        @Override
        public void accept(DexEncodedMethod method) {
          MainDexListBuilder.this.traceMethodDirectDependencies(method.method);
