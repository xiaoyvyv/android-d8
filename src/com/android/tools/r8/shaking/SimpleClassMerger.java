...
public class SimpleClassMerger {
  ...
  private Collection<DexMethod> getInvokes() {
    if (invokes == null) {
      ...
      appInfo.targetedMethods.forEach(new Consumer<DexMethod>() {
        @Override
        public void accept(DexMethod m) {
          SimpleClassMerger.this.addProgramMethods(filteredInvokes, m, equivalence);
        }
      });
      invokes = filteredInvokes.stream().map(new Function<Wrapper<DexMethod>, DexMethod>() {
        @Override
        public DexMethod apply(Wrapper<DexMethod> dexMethodWrapper) {
          return dexMethodWrapper.get();
        }
      }).filter(new Predicate<DexMethod>() {
        @Override
        public boolean test(DexMethod dexMethod) {
          return SimpleClassMerger.this.removeNonProgram(dexMethod);
        }
  ...
  private class ClassMerger {
    ...
    public boolean merge() {
      ...
      Collection<DexEncodedMethod> mergedDirectMethods = mergeItems(
          Iterators.transform(Iterators.forArray(source.directMethods()), new com.google.common.base.Function<DexEncodedMethod, DexEncodedMethod>() {
            @Override
            public DexEncodedMethod apply(DexEncodedMethod method) {
              return ClassMerger.this.renameConstructors(method);
            }
          }),
          target.directMethods(),
          MethodSignatureEquivalence.get(),
          existingMethods,
              new BiFunction<DexEncodedMethod, DexEncodedMethod, DexEncodedMethod>() {
                @Override
                public DexEncodedMethod apply(DexEncodedMethod existing, DexEncodedMethod method) {
                  return ClassMerger.this.renameMethod(existing, method);
                }
              }
      );
      ...
      if (source.accessFlags.isInterface()) {
        ...
        // target class.
        methods = Iterators.transform(methods, new com.google.common.base.Function<DexEncodedMethod, DexEncodedMethod>() {
          @Override
          public DexEncodedMethod apply(DexEncodedMethod m) {
            return ClassMerger.this.filterShadowedInterfaceMethods(m);
          }
        });
      }
      Collection<DexEncodedMethod> mergedVirtualMethods = mergeItems(
          ...
          existingMethods,
              new BiFunction<DexEncodedMethod, DexEncodedMethod, DexEncodedMethod>() {
                @Override
                public DexEncodedMethod apply(DexEncodedMethod existing, DexEncodedMethod method) {
                  return ClassMerger.this.abortOnNonAbstract(existing, method);
                }
              });
      if (abortMerge) {
        return false;
      ...
      Collection<DexEncodedField> mergedStaticFields = mergeItems(
          ...
          existingFields,
              new BiFunction<DexEncodedField, DexEncodedField, DexEncodedField>() {
                @Override
                public DexEncodedField apply(DexEncodedField existing, DexEncodedField field) {
                  return ClassMerger.this.renameField(existing, field);
                }
              });
      ...
      deferredRenamings.forEach(new BiConsumer<DexEncodedMethod, DexEncodedMethod>() {
        @Override
        public void accept(DexEncodedMethod from, DexEncodedMethod to) {
          renamedMembersLense.map(from.method, to.method);
        }
      });
      return true;
    }
      ...
      for (DexMethod method : invokes) {
        ...
        if (positions != 0) {
          Int2IntMap positionsMap =
              seenPositions.computeIfAbsent(method.name, new Function<DexString, Int2IntMap>() {
                @Override
                public Int2IntMap apply(DexString k) {
