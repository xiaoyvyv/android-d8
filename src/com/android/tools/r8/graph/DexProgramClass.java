...
public class DexProgramClass extends DexClass implements Supplier<DexProgramClass> {
  ...
  private boolean hasAnnotations(DexEncodedField[] fields) {
    return fields != null && Arrays.stream(fields).anyMatch(new Predicate<DexEncodedField>() {
      @Override
      public boolean test(DexEncodedField dexEncodedField) {
        return dexEncodedField.hasAnnotation();
      }
    });
  }

  private boolean hasAnnotations(DexEncodedMethod[] methods) {
    return methods != null && Arrays.stream(methods).anyMatch(new Predicate<DexEncodedMethod>() {
      @Override
      public boolean test(DexEncodedMethod dexEncodedMethod) {
        return dexEncodedMethod.hasAnnotation();
      }
    });
  }
