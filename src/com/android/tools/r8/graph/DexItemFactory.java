...
public class DexItemFactory {
  ...
  public void clearSubtypeInformation() {
    types.values().forEach(new Consumer<DexType>() {
      @Override
      public void accept(DexType type) {
        DexType.clearSubtypeInformation(type);
      }
    });
  }
  ...
  public AdvanceLine createAdvanceLine(int delta) {
    synchronized (advanceLines) {
      return advanceLines.computeIfAbsent(delta, new IntFunction<AdvanceLine>() {
        @Override
        public AdvanceLine apply(int delta1) {
          return new AdvanceLine(delta1);
        }
      });
    }
  }
  ...
  public AdvancePC createAdvancePC(int delta) {
    synchronized (advancePCs) {
      return advancePCs.computeIfAbsent(delta, new IntFunction<AdvancePC>() {
        @Override
        public AdvancePC apply(int delta1) {
          return new AdvancePC(delta1);
        }
      });
    }
  }
  ...
  public Default createDefault(int value) {
    synchronized (defaults) {
      return defaults.computeIfAbsent(value, new IntFunction<Default>() {
        @Override
        public Default apply(int value1) {
          return new Default(value1);
        }
      });
    }
  }
  ...
  public EndLocal createEndLocal(int registerNum) {
    synchronized (endLocals) {
      return endLocals.computeIfAbsent(registerNum, new IntFunction<EndLocal>() {
        @Override
        public EndLocal apply(int registerNum1) {
          return new EndLocal(registerNum1);
        }
      });
    }
  }
  ...
  public RestartLocal createRestartLocal(int registerNum) {
    synchronized (restartLocals) {
      return restartLocals.computeIfAbsent(registerNum, new IntFunction<RestartLocal>() {
        @Override
        public RestartLocal apply(int registerNum1) {
          return new RestartLocal(registerNum1);
        }
      });
    }
  }
  ...
  public SetFile createSetFile(DexString fileName) {
    synchronized (setFiles) {
      return setFiles.computeIfAbsent(fileName, new Function<DexString, SetFile>() {
        @Override
        public SetFile apply(DexString fileName1) {
          return new SetFile(fileName1);
        }
      });
    }
  }
  ...
  private static <S extends PresortedComparable<S>> void assignSortedIndices(Collection<S> items,
    ...
    sorted.sort(new Comparator<S>() {
      @Override
      public int compare(S a, S b) {
        return a.layeredCompareTo(b, namingLens);
      }
    });
    int i = 0;
    for (S value : sorted) {
  ...
  synchronized public void resetSortedIndices() {
    ...
    strings.values().forEach(new Consumer<DexString>() {
      @Override
      public void accept(DexString dexString) {
        dexString.resetSortedIndex();
      }
    });
    types.values().forEach(new Consumer<DexType>() {
      @Override
      public void accept(DexType dexType) {
        dexType.resetSortedIndex();
      }
    });
    fields.values().forEach(new Consumer<DexField>() {
      @Override
      public void accept(DexField dexField) {
        dexField.resetSortedIndex();
      }
    });
    protos.values().forEach(new Consumer<DexProto>() {
      @Override
      public void accept(DexProto dexProto) {
        dexProto.resetSortedIndex();
      }
    });
    methods.values().forEach(new Consumer<DexMethod>() {
      @Override
      public void accept(DexMethod dexMethod) {
        dexMethod.resetSortedIndex();
      }
    });
    sorted = false;
  }
