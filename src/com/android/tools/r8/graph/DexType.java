...
public class DexType extends IndexedDexItem implements PresortedComparable<DexType> {
  ...
  public boolean isSubtypeOf(DexType other, AppInfo appInfo) {
    ...
    if (other.hierarchyLevel == INTERFACE_LEVEL) {
      return other.directSubtypes.stream().anyMatch(new Predicate<DexType>() {
        @Override
        public boolean test(DexType subtype) {
          return DexType.this.isSubtypeOf(subtype,
                  appInfo);
        }
      });
    }
  ...
  @Override
  public <T extends PresortedComparable<T>> int slowCompare(T a, T b) {
