...
public class AppInfoWithSubtyping extends AppInfo {
  ...
  private void populateSuperType(Hashtable<DexType, Set<DexType>> map, DexType superType,
    ...
    if (superType != null) {
      Set<DexType> set = map.computeIfAbsent(superType, new Function<DexType, Set<DexType>>() {
        @Override
        public Set<DexType> apply(DexType ignore) {
          return new HashSet<>();
        }
      });
  ...
  private void populateSubtypeMap(Map<DexType, DexClass> classes, DexItemFactory dexItemFactory) {
    ...
    for (Map.Entry<DexType, DexClass> entry : classes.entrySet()) {
      populateAllSuperTypes(map, entry.getKey(), entry.getValue(), new Function<DexType, DexClass>() {
        @Override
        public DexClass apply(DexType o) {
          return classes.get(o);
        }
      });
    }
    ...
    if (!DexType.validateLevelsAreCorrect(new Function<DexType, DexClass>() {
      @Override
      public DexClass apply(DexType o) {
        return classes.get(o);
      }
    }, dexItemFactory)) throw new AssertionError();
  }
